...
const overrides = {
  ...
  'heco-bifi-maxi': { rabityieldFeeRecipient: undefined }, // 0x0
  'polygon-bifi-maxi': { rabityieldFeeRecipient: undefined }, // 0x0
  'avax-bifi-maxi': { rabityieldFeeRecipient: undefined }, // 0x0
  'bifi-maxi': { stratOwner: undefined }, // harvester 0xDe30
  'beltv2-4belt': { vaultOwner: undefined }, // moonpot deployer
  'cronos-bifi-maxi': { rabityieldFeeRecipient: undefined }, // 0x0
  'metis-bifi-maxi': { rabityieldFeeRecipient: undefined }, // 0x0
  'aurora-bifi-maxi': { rabityieldFeeRecipient: undefined }, // 0x0
  'fuse-bifi-maxi': { rabityieldFeeRecipient: undefined }, // 0x0
  'moonbeam-bifi-maxi': { rabityieldFeeRecipient: undefined }, // 0x0
  'scream-frax': { vaultOwner: undefined }, // Rescue
};

const oldValidOwners = [
  addressBook.fantom.platforms.rabityieldfinance.devMultisig,
  addressBook.polygon.platforms.rabityieldfinance.devMultisig,
  addressBook.arbitrum.platforms.rabityieldfinance.devMultisig,
];
...
const validatePools = async () => {
  ...
  for (let [chain, pools] of Object.entries(chainPools)) {
    ...
    pools.forEach(pool => {
      ...
      const { keeper, strategyOwner, vaultOwner, rabityieldFeeRecipient } =
        addressBook[chain].platforms.rabityieldfinance;
      ...
      updates = isRabityieldFeeRecipientCorrect(pool, chain, rabityieldFeeRecipient, updates);
    });
    if (!isEmpty(updates)) {
      exitCode = 1;
    }
...
const isRabityieldFeeRecipientCorrect = (pool, chain, recipient, updates) => {
  const validRecipients = oldValidFeeRecipients[chain] || [];
  if (
    pool.status === 'active' &&
    pool.rabityieldFeeRecipient !== undefined &&
    pool.rabityieldFeeRecipient !== recipient &&
    !validRecipients.includes(pool.rabityieldFeeRecipient)
  ) {
    console.log(
      `Pool ${pool.id} should update rabityield fee recipient. From: ${pool.rabityieldFeeRecipient} To: ${recipient}`
    );

    if (!('rabityieldFeeRecipient' in updates)) updates['rabityieldFeeRecipient'] = {};
    if (!(chain in updates.rabityieldFeeRecipient)) updates.rabityieldFeeRecipient[chain] = {};

    if (pool.stratOwner in updates.rabityieldFeeRecipient[chain]) {
      updates.rabityieldFeeRecipient[chain][pool.stratOwner].push(pool.strategy);
    } else {
      updates.rabityieldFeeRecipient[chain][pool.stratOwner] = [pool.strategy];
    }
  }
...
const populateVaultsData = async (chain, pools, web3) => {
  const multicall = new MultiCall(web3, addressBook[chain].platforms.rabityieldfinance.multicall);
...
const populateStrategyData = async (chain, pools, web3) => {
  ...
  const calls = pools.map(pool => {
    ...
    return {
      ...
      rabityieldFeeRecipient: stratContract.methods.rabityieldFeeRecipient(),
  ...
  return pools.map((pool, i) => {
    return {
      ...
      rabityieldFeeRecipient: results[i].rabityieldFeeRecipient,
